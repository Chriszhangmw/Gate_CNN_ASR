...
def train(
    ...
    dev_dataloader = data.MASRDataLoader(
        dev_dataset, batch_size=batch_size, num_workers=4
    )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (x, y, x_lens, y_lens) in enumerate(train_dataloader):
            x = x.to("cuda")
            # out1,out2, out_lens = model(x, x_lens)
        ...
        if cer < best_cer:
            torch.save(model, "/home/zmw/big_space/zhangmeiwei_space/asr_res_model/masr/ad/model_best_drop2_a0.1.pth")
            best_cer = cer
        if epoch > 100:
...
def eval_test(model,dataloader):
    ...
    model.train()
    print("在测试集上的CER值为： ",cer,f1, bleu_1, bleu_2, unigrams_distinct, bigrams_distinct, intra_dist1, intra_dist2)
    return cer
