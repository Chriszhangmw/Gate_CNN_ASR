...
class GatedConv(MASRModel):
    ...
    def __init__(self, vocabulary, blank=0, name="masr"):
        ...
        modules.append(ConvBlock(nn.Conv1d(161, 500, 48, 2, 97), 0.1))

        for i in range(7):
            # modules.append(ConvBlock(nn.Conv1d(250, 500, 7, 1), 0.3))
            modules.append(ConvBlock(nn.Conv1d(250, 500, 7, 1), 0.1))

        # modules.append(ConvBlock(nn.Conv1d(250, 2000, 32, 1), 0.5))
        modules.append(ConvBlock(nn.Conv1d(250, 2000, 32, 1), 0.1))

        # modules.append(ConvBlock(nn.Conv1d(1000, 2000, 1, 1), 0.5))
        modules.append(ConvBlock(nn.Conv1d(1000, 2000, 1, 1), 0.1))
    ...
    def predict2(self, path,decoder):
        self.eval()
        wav = feature.load_audio(path)
        spec = feature.spectrogram(wav)
        spec.unsqueeze_(0)
        x_lens = spec.size(-1)
        out,x_lens2 = self.forward(spec,x_lens)
        # out_len = torch.tensor([out.size(-1)])
        out = F.softmax(out, 1)
        out = out.transpose(1, 2)
        # print(x_lens2)
        x_lens2 = [x_lens2]
        out_strings, _ = decoder.decode(out, x_lens2)
        self.train()
        return out_strings[0]
